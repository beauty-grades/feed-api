...
const correct_handle = (handle: string) => {
  let corrected_handle: string;
  if (handle.endsWith(" 1")) {
    corrected_handle = handle.slice(0, -2);
  } else {
    corrected_handle = handle;
  }

  return corrected_handle.replace(/[\s()._-]/g, "");
};

export const parseEvaluations = ({
  rawScores,
  formula,
}: {
  rawScores: RawScore[];
  formula: string;
}): { evaluations: Map<string, Evaluation>; wrong_formula: boolean } => {
  let weights: {
    id: string;
    weight: number | null;
  }[] = [];
  ...
  if (weights.length > 1 && found_100) {
    ...
    rawScores.forEach((score) =>
      evaluations.set(correct_handle(score.code), {
  ...
  for (const weight of weights) {
    ...
    switch (matching_scores.length) {
              ...
              default:
                ...
                matching_scores.forEach(
                  ({ handle, can_be_deleted, raw_weight, score }) => {
                    evaluations.set(handle.replace(/[\s()._-]/g, ""), {
                      label: weight.id,
                      can_be_deleted,
                      weight: weight.weight
                        ? roundTo(
      ...
      case 1:
        evaluations.set(weight.id.replace(/[\s()._-]/g, ""), {
