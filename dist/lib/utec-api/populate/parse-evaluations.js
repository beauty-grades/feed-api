...
export const parseEvaluations = (rawScores, formula) => {
    let weights = [];
    let evaluations = [];
    let found_100 = false;
    let match;
    ...
    for (const weight of weights) {
        let matching_scores = scores_to_check.filter(({ handle }) => handle.includes(weight.id));
        switch (matching_scores.length) {
            case 0:
                matching_scores = scores_to_check.filter(({ handle }) => handle.includes(weight.id.replace("Prom.", "")));
                switch (matching_scores.length) {
                    case 0:
                        matching_scores = scores_to_check.filter(({ label }) => label.includes(weight.id));
                        switch (matching_scores.length) {
                            case 0:
                                // no matching scores
                                console.log(formula);
                            ...
                            case 1:
                                ...
                                scores_to_check.splice(scores_to_check.findIndex(({ handle: handle_to_check }) => handle_to_check === matching_scores[0].handle), 1);
                                break;
                            default:
                                const sum_of_weights = matching_scores.reduce((acc, { raw_weight }) => acc + raw_weight, 0);
                                matching_scores.forEach(({ handle, can_be_deleted, raw_weight, score }) => {
                                    evaluations.push({
                                        handle: handle.replace(" ", ""),
                                        label: weight.id,
                                        can_be_deleted,
                                        weight: weight.weight
                                            ? roundTo((weight.weight * raw_weight) / sum_of_weights, 4)
                                            : null,
                                        score,
                                    });
                                    // remove from scores_to_check
                                    scores_to_check.splice(scores_to_check.findIndex(({ handle: handle_to_check }) => handle_to_check === handle), 1);
                                });
                                break;
                        }
                        break;
                    case 1:
                        ...
                        break;
                    default:
                        matching_scores.forEach(({ handle, label, can_be_deleted, score }) => {
                            evaluations.push({
                                handle: handle.replace(" ", ""),
                                label,
                                can_be_deleted,
            ...
            case 1:
                ...
                // remove from scores_to_check
                scores_to_check.splice(scores_to_check.findIndex(({ handle }) => handle === matching_scores[0].handle), 1);
                break;
            default:
                const total_weight_sum = matching_scores.reduce((acc, { raw_weight }) => acc + raw_weight, 0);
                matching_scores.forEach(({ handle, label, can_be_deleted, raw_weight, score }) => {
                    evaluations.push({
                        handle: handle.replace(" ", ""),
                        label,
                        can_be_deleted,
