...
import { LocalClase, LocalClassroom, LocalCourse, LocalCurriculum, LocalEnrollment, LocalEvaluation, LocalGrade, LocalLevel, LocalLevel_Course, LocalPeriod, LocalStudent, LocalStudent_Curriculum, LocalTeacher, } from "./classes";
import { CustomExecuteParallel } from "./custom-execute-parallel";
import { parseEvaluations } from "./parse-evaluations";
import { retryFetch } from "./retry-fetch";
export const populate = async (utec_token_v1, email) => {
    const LocalRecords = {
        curriculums: [],
        periods: [],
        courses: [],
    ...
    await Promise.all([
        ...
        (async () => {
            ...
            await Promise.all(fetched_curriculums.map(async ({ academicProgramId, tittle }) => {
                ...
                const current_student_curriculum = new LocalStudent_Curriculum(LocalRecords.student, current_curriculum);
                ...
                fetched_levels.map(({ title, courses: raw_courses }) => {
                    const current_level = new LocalLevel(parseInt(title.split(" ")[1]), current_curriculum);
                    ...
                    raw_courses.forEach((course) => {
                        if (course.isElective) {
                            ...
                            let classroom_score = course.summaryEnrolled[0]?.classroomAverage;
                            let xd_period = course.summaryEnrolled[0]?.namePeriod.replace(/\s/g, "");
                            if (classroom_score) {
                            ...
                            let current_course = LocalRecords.courses.find((course) => course.handle === parsed_handle);
                            if (!current_course) {
                                current_course = new LocalCourse(parsed_handle, parsed_name);
                                LocalRecords.courses.push(current_course);
                            }
                            const current_level_course = new LocalLevel_Course(current_level, current_course, course.credits);
        ...
        (async () => {
            ...
            const handlePeriod = async ({ codPeriodo, descPeriodo }) => {
                ...
                const res_get_period = await retryFetch(`https://api.utec.edu.pe/academico-api/alumnos/me/course/details`, {
                    method: "POST",
                    headers: {
                        "x-auth-token": utec_token_v1,
                        "Content-Type": "application/json",
                ...
                if (fetched_courses.length && fetched_courses.length > 0) {
                    fetched_courses.forEach((course) => {
                        ...
                        let current_course = LocalRecords.courses.find((c) => c.handle === parsed_handle);
                        if (!current_course) {
                        ...
                        let current_teacher = LocalRecords.teachers.find((teacher) => teacher.first_name === first_name &&
                            teacher.last_name === last_name);
                        if (!current_teacher) {
                            current_teacher = new LocalTeacher(first_name, last_name);
                            LocalRecords.teachers.push(current_teacher);
                        }
                        const current_classroom = new LocalClassroom(parseInt(course.sectionName), current_clase, current_teacher);
                        ...
                        const current_enrollment = new LocalEnrollment(LocalRecords.student, current_classroom, final_score, dropped_out, elective);
                        LocalRecords.enrollments.push(current_enrollment);
                        if (course.scores.length && course.scores.length > 0) {
                            const { evaluations: parsed_evaluations, wrong_formula } = parseEvaluations(course.scores, course.formula);
                            if (wrong_formula) {
                                current_clase.setWrongFormula();
                            }
                            parsed_evaluations.forEach(({ handle, label, weight, can_be_deleted, score }) => {
                                const current_evaluation = new LocalEvaluation(handle, label, weight, can_be_deleted, current_clase);
                                LocalRecords.evaluations.push(current_evaluation);
                                if (score) {
                                    const current_score = new LocalGrade(current_evaluation, current_enrollment, score);
            ...
            console.log(`Fetch periods took: ${minutes}:${seconds.toString().padStart(2, "0")}`);
        })(),
    ]);
    temp_classrooms_scores.forEach((temp_classroom) => {
        const matched_classroom = LocalRecords.classrooms.find((classroom) => classroom.clase.course.handle === temp_classroom.course_handle &&
    ...
    await Promise.all([
        ...
        (async () => {
            XataRecords.curriculums = await Promise.all(LocalRecords.curriculums.map(async (local_curriculum) => {
        ...
        // Register Periods
        (async () => {
            XataRecords.periods = await Promise.all(LocalRecords.periods.map(async (local_period) => {
        ...
        // Register Courses
        (async () => {
            XataRecords.courses = await Promise.all(LocalRecords.courses.map(async (local_course) => {
        ...
        // Register Teachers
        (async () => {
            XataRecords.teachers = await Promise.all(LocalRecords.teachers.map(async (local_teacher) => {
    ...
    await Promise.all([
        // Register Levels
        (async () => {
            XataRecords.levels = await Promise.all(LocalRecords.levels.map(async (loca_level) => {
                const xata_curriculum_matching = XataRecords.curriculums.find((xata_curriculum) => xata_curriculum.handle === loca_level.curriculum.handle);
        ...
        // Register Classes
        (async () => {
            XataRecords.classes = await Promise.all(LocalRecords.clases.map(async (local_clase) => {
                const xata_course_matching = XataRecords.courses.find((xata_course) => xata_course.handle === local_clase.course.handle);
                if (!xata_course_matching) {
                    throw new Error("Course not found");
                }
                const xata_period_matching = XataRecords.periods.find((xata_period) => xata_period.handle === local_clase.period.handle);
        ...
        (async () => {
            XataRecords.student_curriculums = (await Promise.all(LocalRecords.student_curriculums.map(async (student_curriculum) => {
                const xata_student = XataRecords.student;
                if (!xata_student) {
                    throw new Error("Student not found");
                }
                const xata_curriculum_matching = XataRecords.curriculums.find((xata_curriculum) => xata_curriculum.handle === student_curriculum.curriculum.handle);
                if (!xata_curriculum_matching) {
                    throw new Error("Curriculum not found");
                }
    ...
    await Promise.all([
        ...
        (async () => {
            XataRecords.level_courses = await Promise.all(LocalRecords.level_courses.map(async (level_course) => {
                const xata_level_matching = XataRecords.levels.find((xata_level) => xata_level.order === level_course.level.order &&
                    xata_level.curriculum?.handle ===
                        level_course.level.curriculum.handle);
                if (!xata_level_matching) {
                    throw new Error("Level not found");
                }
                const xata_course_matching = XataRecords.courses.find((course) => course.handle === level_course.course.handle);
                if (!xata_course_matching) {
                    throw new Error("Course not found");
                }
        ...
        (async () => {
            XataRecords.evaluations = (await Promise.all(LocalRecords.evaluations.map(async (evaluation) => {
                const xata_class = XataRecords.classes.find((clase) => {
                    ...
                    const xata_period = XataRecords.periods.find((period) => period.id === xata_period_id);
        ...
        // Register Classrooms
        (async () => {
            XataRecords.classrooms = await Promise.all(LocalRecords.classrooms.map(async (classroom) => {
                const xata_class = XataRecords.classes.find((clase) => {
                    const xata_course_id = clase.course?.id || "123";
                    const xata_course = XataRecords.courses.find((course) => course.id === xata_course_id);
                    const xata_period_id = clase.period?.id || "123";
    ...
    XataRecords.enrollments = await Promise.all(LocalRecords.enrollments.map(async (local_enrollment) => {
        const xata_student = XataRecords.student;
        if (!xata_student) {
            throw new Error("Student not found");
        }
    ...
    XataRecords.grades = await Promise.all(LocalRecords.grades.map(async (score) => {
        ...
        const xata_evaluation = XataRecords.evaluations.find((evaluation) => {
            ...
            const xata_period_id = xata_class?.period?.id || "123";
            const xata_period = XataRecords.periods.find((period) => period.id === xata_period_id);
            return (xata_course?.handle === score.evaluation.clase.course.handle &&
